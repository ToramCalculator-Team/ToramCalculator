...
- **修订**：纠错、删除误写入的实现细节、补关系、改变状态或用新 ADR 取代旧 ADR。
...
ADR 只记录长期架构边界、可行方案及其取舍、确立的不变量、接受的代价和重新评估条件。写作或审阅时，必须把下列内容排除在 ADR 正文之外：

**字段名、状态枚举、偏移、容量、公式、具体表结构、类名和迁移步骤都不属于 ADR，应由代码契约、测试或实施计划表达。**

- 字段名、状态枚举、偏移、容量数值与容量公式；
- 具体表结构、类型变体、类名、函数名、API 和算法；
- 逐文件修改、迁移步骤、命令、验收清单、负责人和进度。

字段、类型、schema、算法和可执行约束由代码契约、测试或必要的实现注释表达；实施顺序、首版范围和验收由计划、Issue 或 PR 表达。不得因为内容跨越多个模块，就把实现细节提升为 ADR 决议。若某条 ADR 已误写入这些内容，应在不改变长期架构边界的前提下直接修订并留下简短说明。
...
- `Accepted` ADR 默认是历史证据。架构问题、候选方案或长期边界发生变化时，创建新 ADR 并建立 `Supersedes` 双向关系。
- 若架构问题与长期边界未变，只是删除错误的字段、算法、类型变体、迁移步骤等实现细节，或让正文重新匹配已经确认的同一架构边界，可以直接修订原 ADR。修订必须由用户明确确认，并在 ADR 中留下简短修订说明；不得借此改写真实存在过的架构取舍。
