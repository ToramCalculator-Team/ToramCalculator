...
export class RendererCommunication {
	private source: SimulationRenderSource | null = null;
	 ...
	 * 初始化渲染通信
	 */
	initialize(source: SimulationRenderSource) {
		...
		source.on("render_cmd", this.boundHandleRenderCommand);
	...
	dispose() {
		...
		// 移除事件监听（使用保存的引用）
		if (this.boundHandleRenderCommand && this.source) {
			this.source.off("render_cmd", this.boundHandleRenderCommand);
	...
	private handleRenderCommand(data: { engineId: string; cmd: unknown }) {
		...
		const payload =
			typeof renderData === "object" && renderData !== null
