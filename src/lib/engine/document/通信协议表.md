...
> 历史快照提示：当前线程协议的权威来源是 `src/lib/engine/core/thread/protocol.ts`。本文只保留通信设计脉络和人工导读，新增或修改顶层消息类型时先改代码 schema，再同步本文。
...
|---|---|---|---|---|---|---|---|---|---|
| 引擎生命周期快照 | `engine_lifecycle_snapshot` | Worker → Main | 低频/变化驱动 | 否 | `EngineLifecycleSnapshot` | `Simulation.worker.ts` → `postSystemMessage` | `EngineWorkerClient` / lifecycle 订阅方 | ✅ 已实现 | 保持 |
...
| 控制器领域事件批 | `domain_event_batch` | Worker → Main | 低频/变化驱动 | 否 | `{type:"controller_domain_event_batch", tickIndex, events}` | `ControllerEventProjector` → `postSystemMessage` | `EngineWorkerClient` / UI 订阅方 | ✅ 已实现 | 保持 |
| 引擎遥测（轻量） | `engine_telemetry` | Worker → Main | 中频（默认 200ms） | 否 | `{tickIndex, currentTimeMs, runTime, ticksPerSecond, memberCount}` | `startTelemetryLoop` → `postSystemMessage` | `EngineWorkerClient` / UI 订阅方 | ✅ 已实现 | 保持 |
...
- ✅ 已完成：`domain_event_batch` 已为顶层 type；引擎生命周期推送已统一为 `engine_lifecycle_snapshot`。
