...
export type WorkerSystemMessageEnvelope = z.output<typeof WorkerSystemMessageEnvelopeSchema>;

/** Worker 对单个池任务的响应信封；业务结果保持 unknown，由上层请求对应的 Schema 继续解析。 */
export const WorkerTaskResponseEnvelopeSchema = z.object({
	belongToTaskId: z.string(),
...
export type WorkerTaskResponseEnvelope = z.output<typeof WorkerTaskResponseEnvelopeSchema>;
...
/** 通用任务池结果；业务 payload 本身可以再使用自己的判别联合。 */
export type Result<T> =
	| { success: true; data: T; metrics?: TaskMetrics }
	| { success: false; error: string; metrics?: TaskMetrics };
 ...
 */
export type WorkerMessageEvent<TResult, TTaskTypeMap extends object, TData> = {
