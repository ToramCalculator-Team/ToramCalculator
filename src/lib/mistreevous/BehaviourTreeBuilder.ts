...
import type { Composite } from "./nodes/composite/Composite";
...
import type { Decorator } from "./nodes/decorator/Decorator";
...
function applyLeafNodeGuardPaths(root: Root) {
	...
	const findLeafNodes = (path: Node[], node: Node) => {
		...
		if (node instanceof Leaf) {
			...
			(node as Composite | Decorator).getChildren().forEach((child) => {
