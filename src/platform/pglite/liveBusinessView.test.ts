...
describe("local-first business view 的 live 回流", () => {
	it("覆盖本地写、synced 回灌、后续远端更新和 local rollback", async () => {
		...
		const subscription = await db.live.query<{ id: string; name: string }>(
			`SELECT id, name FROM activity WHERE id = $1`,
			["live-business-view"],
		...
		await db.query(
			`INSERT INTO activity (id, name, "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId")
			 VALUES ($1, 'name-1', NOW(), NOW(), NULL, NULL)`,
			["live-business-view"],
		);
		await waitFor(() => snapshots.find((rows) => rows[0]?.name === "name-1"));

		const local = await db.query<{ write_id: string }>(`SELECT write_id::text FROM activity_local WHERE id = $1`, [
			"live-business-view",
		]);
		...
		await db.query(
			`INSERT INTO activity_synced (id, name, "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId", write_id)
			 VALUES ($1, 'name-1', NOW(), NOW(), NULL, NULL, $2)`,
			["live-business-view", local.rows[0].write_id],
		);
		await waitFor(() => (snapshots.length > syncedSnapshotStart ? snapshots.at(-1) : undefined));
		expect(snapshots.at(-1)?.[0]?.name).toBe("name-1");

		await db.query(`UPDATE activity_synced SET name = 'name-2' WHERE id = $1`, ["live-business-view"]);
		await waitFor(() => snapshots.find((rows) => rows[0]?.name === "name-2"));

		await db.query(`UPDATE activity SET name = 'name-3' WHERE id = $1`, ["live-business-view"]);
		await waitFor(() => snapshots.find((rows) => rows[0]?.name === "name-3"));

		// 模拟服务端拒绝后同步层清理 local 覆盖，business view 应回到 synced=2。
		const rollbackSnapshotStart = snapshots.length;
		await db.query(`DELETE FROM activity_local WHERE id = $1`, ["live-business-view"]);
		await waitFor(() => snapshots.slice(rollbackSnapshotStart).find((rows) => rows[0]?.name === "name-2"));

		expect(snapshots.at(-1)).toEqual([{ id: "live-business-view", name: "name-2" }]);
