 ...
 *
 * 载体选用 activity（TEXT 主键 + 基础审计列），结构轻量且生成模式与业务表一致。
 */
...
describe("local-first 写链路收敛(基于生成的 client.sql)", () => {
	it("基线:通过视图 INSERT 写入 _local 覆盖行并记录 changes 日志", async () => {
		await db.query(
			`INSERT INTO "activity" ("id", "name", "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId")
			 VALUES ('activity-a', 'name-1', NOW(), NOW(), NULL, NULL)`,
		);

		const local = await db.query<{ id: string; write_id: string }>(
			`SELECT id, write_id::text FROM "activity_local" WHERE id = 'activity-a'`,
		);
		...
		const changes = await db.query<{ operation: string; write_id: string; value: Record<string, unknown> }>(
			`SELECT operation, write_id::text, value FROM changes WHERE table_name = 'activity'`,
		);
		...
		// value 快照只含业务列
		expect(Object.keys(changes.rows[0].value).sort()).toEqual([
			"createdAt",
			"createdByAccountId",
			"id",
			"name",
	...
	it("收敛:回灌行携带 write_id(修复后服务端会持久化回传)时,_local 立即被清、视图反映 synced", async () => {
		const local = await db.query<{ write_id: string }>(
			`SELECT write_id::text FROM "activity_local" WHERE id = 'activity-a'`,
		);
		const writeId = local.rows[0].write_id;

		// 模拟 Electric 从服务端回灌:服务端已持久化 write_id(B-1),故回灌行带上它
		await db.query(
			`INSERT INTO "activity_synced" ("id", "name", "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId", "write_id")
			 VALUES ('activity-a', 'name-1', NOW(), NOW(), NULL, NULL, $1)`,
			[writeId],
		);

		// 清理触发器 `write_id = NEW.write_id` 命中 → local 覆盖行被删除
		const localAfter = await db.query(`SELECT id FROM "activity_local" WHERE id = 'activity-a'`);
		expect(localAfter.rows).toHaveLength(0);
	});

	it("写后读一致:收敛后服务端(其它设备)的后续更新不再被 _local 遮蔽", async () => {
		// 另一台设备把 name 改成 server-999 并经服务端同步回来
		await db.query(`UPDATE "activity_synced" SET "name" = 'server-999' WHERE "id" = 'activity-a'`);

		// local 已清,视图直接反映 synced 的最新值
		const view = await db.query<{ name: string }>(`SELECT name FROM "activity" WHERE id = 'activity-a'`);
		expect(view.rows[0].name).toBe("server-999");
	});

	it("回归对照:回灌 write_id=NULL(修复前的服务端现状)仍无法清理 —— 证明缺口确实在服务端持久化", async () => {
		await db.query(
			`INSERT INTO "activity" ("id", "name", "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId")
			 VALUES ('activity-c', 'name-5', NOW(), NOW(), NULL, NULL)`,
		);
		// write_id=NULL 模拟"服务端没持久化 write_id"的旧行为
		await db.query(
			`INSERT INTO "activity_synced" ("id", "name", "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId", "write_id")
			 VALUES ('activity-c', 'name-5', NOW(), NOW(), NULL, NULL, NULL)`,
		);
		// 清理条件含 `write_id IS NOT NULL`,NULL 不命中 → local 残留(印证 B-1 修复的必要性)
		const local = await db.query(`SELECT id FROM "activity_local" WHERE id = 'activity-c'`);
		expect(local.rows).toHaveLength(1);
	});
	...
	it("视图 DELETE 产生 operation='delete' 上行变更(服务端现已接受,见 B-2)", async () => {
		await db.query(
			`INSERT INTO "activity" ("id", "name", "createdAt", "updatedAt", "updatedByAccountId", "createdByAccountId")
			 VALUES ('activity-d', 'name-7', NOW(), NOW(), NULL, NULL)`,
		);
		await db.query(`DELETE FROM "activity" WHERE "id" = 'activity-d'`);

		const changes = await db.query<{ operation: string }>(
			`SELECT operation FROM changes WHERE table_name = 'activity' ORDER BY id`,
		);
