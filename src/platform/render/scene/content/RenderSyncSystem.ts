 ...
 * 无 reader 的控制器只服务静态内容及离散命令，不提供第二套实时运动降级路径。
 */

import {
	type MemberSlotIndex,
	STATE_FLAG_AIRBORNE,
	STATE_FLAG_MOVING,
...
export class RenderSyncSystem {
	...
	/**
	 * @param worldStateReader SAB 世界状态读取器（可选）；提供时每帧读取权威位置、朝向和移动标志。
	 * @param slotIndex memberId → SAB 槽位下标映射（与 worldStateReader 同生命周期）。
	 */
	constructor(
		private worldStateReader?: WorldStateReader | null,
		private slotIndex?: MemberSlotIndex | null,
	) {}
	 ...
	 */
	syncEntities(entities: Map<string, EntityRuntime>, dtSec: number): void {
		const reader = this.worldStateReader;
		...
		entities.forEach((entity) => {
			...
			if (reader && index) {
				...
				const slotIdx = index.get(entity.id);
				if (slotIdx !== undefined) {
					const slot = reader.readSlot(slotIdx);
					if (slot) {
						stateFlags = slot.stateFlags;
						// 更新 entity.physics（统一真相，让相机跟随与拾取读到正确值）。
						entity.physics.pos.x = slot.posX;
						entity.physics.pos.y = slot.posY;
						entity.physics.pos.z = slot.posZ;
