...
import { findItemWithCrystalById, findCrystals, Crystal, createCrystal } from "~/repositories/crystal";
...
export const createCrystalDataConfig = (
  dic: Dic<crystalWithItem>,
): dataDisplayConfig<crystalWithItem, CrystalCard, {}> => ({
  table: {
    columnDef: [
      {
        accessorKey: "id",
    ...
    dataFetcher: findCrystals,
    defaultSort: { id: "itemType", desc: false },
    hiddenColumnDef: ["id", "itemId"],
    dictionary: dic,
    tdGenerator: (props: { cell: Cell<crystalWithItem, keyof crystalWithItem> }) => {
      const [tdContent, setTdContent] = createSignal<JSX.Element>(<>{"=.=.=.="}</>);
      let defaultTdClass = "text-main-text-color flex flex-col justify-center px-6 py-3";
      const columnId = props.cell.column.id as keyof crystalWithItem;
      switch (columnId) {
        default:
        ...
        >
          <Show when={true} fallback={tdContent()}>
            {"enumMap" in dic.fields[columnId]
              ? (dic.fields[columnId] as EnumFieldDetail<keyof crystalWithItem>).enumMap[props.cell.getValue()]
              : props.cell.getValue()}
          </Show>
        </td>
      );
  ...
  form: {
    ...
    fieldGenerators: {
      type: (key, field) => {
        const zodValue = crystalSchema.shape[key];
        return (
          <EnumSelect
    ...
    onSubmit: async (data) => {
      ...
      const crystal = await db.transaction().execute(async (trx) => {
        ...
        const crystalData = omit(data, Object.keys(defaultData.item) as (keyof item)[]);
  ...
  card: {
    dataFetcher: findItemWithCrystalById,
    cardRender: (data, appendCardTypeAndIds) => {
      ...
      return (
        <>
          {DBDataRender<CrystalCard>({
            data,
            dictionary: dic,
            dataSchema: crystalWithItemSchema,
            hiddenFields: ["id", "itemId", "itemType", "createdByAccountId", "updatedByAccountId"],
            fieldGroupMap: {
