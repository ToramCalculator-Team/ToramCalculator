...
export default function CharactePage() {
  ...
  return (
    <div class="Content flex h-full w-full flex-1 flex-col gap-6 p-6">
      <canvas ref={canvas!} class="CharacterView border-dividing-color h-full w-full rounded-md border-1">
      ...
      >
        <div class={`flex flex-row gap-2`}>
          <Button onClick={() => console.log("连击")} level="quaternary" icon={<Icons.Outline.Gamepad />} class="flex-none">
            {dictionary().db.combo.selfName}
          </Button>
          <Button onClick={() => console.log("装备")} level="quaternary" icon={<Icons.Outline.Category />} class="flex-none">
            装备
          </Button>
          <Button onClick={() => console.log("消耗品")} level="quaternary" icon={<Icons.Outline.Sale />} class="flex-none">
            {dictionary().db.consumable.selfName}
          </Button>
          <Button onClick={() => console.log("料理")} level="quaternary" icon={<Icons.Outline.Coupon2 />} class="flex-none">
            料理
          </Button>
          <Button onClick={() => console.log("雷吉斯托环")} level="quaternary" icon={<Icons.Outline.CreditCard />} class="flex-none">
            雷吉斯托环
          </Button>
          <Button onClick={() => console.log("技能")} level="quaternary" icon={<Icons.Outline.Scale />} class="flex-none">
            技能
          </Button>
          <Button onClick={() => console.log("能力值")} level="quaternary" icon={<Icons.Outline.Filter />} class="flex-none">
            能力值
          </Button>
          <Button onClick={() => console.log("基本配置")} level="quaternary" icon={<Icons.Outline.Edit />} class="flex-none">
            基本配置
          </Button>
        </div>
      </OverlayScrollbarsComponent>
      <div class={`flex w-full flex-col gap-3`}>
        <div class={`flex h-[448px] w-full flex-col gap-3`}></div>
      </div>
    </div>
  );
}
