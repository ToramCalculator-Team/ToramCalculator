...
export const SkillPage = (dic: Dictionary, itemHandleClick: (id: string) => void) => {
	const [expandedSkilles, setExpandedSkilles] = createSignal<Set<string>>(new Set());
	...
	const filteredSkilles = createMemo(() => {
	...
	const gridInfo = createMemo(() => {
		const addressesData = filteredSkilles();
										...
										>
											...
											<Show when={address.zones && address.zones.length > 0}>
												<div class="Zones flex w-full flex-col items-start justify-start gap-1">
													{(expandedSkilles().has(address.id) ? address.zones : address.zones.slice(0, 4)).map(
														(zone) => (
													...
													{address.zones.length > 4 && (
														...
														>
															{expandedSkilles().has(address.id) ? (
