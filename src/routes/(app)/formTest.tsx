...
import { createMemo, createResource, For, onMount, type ParentProps, Show, useContext } from "solid-js";
import { Motion, Presence } from "solid-motionone";
import { DBForm } from "~/components/business/form/DBFormRenderer";
import { Input } from "~/components/controls/input";
import { Form, fieldInfo } from "~/components/dataDisplay/form";
	...
	return (
		<Motion.main class="flex h-full w-full flex-col-reverse landscape:flex-row">
			...
			>
				<Show when={character()}>
					{(varCharacter) => (
						<DBForm tableName="character" initialValue={varCharacter()} dataSchema={CharacterSchema} />
					)}
				</Show>
			</OverlayScrollbarsComponent>
		</Motion.main>
