...
> 本目录保留引擎设计讨论和历史叙述。当前实现契约以代码、schema、测试和 `docs/decisions/README.md` 的当前决策视图为准；阅读本目录时不要把旧协议名、旧字段名或旧 Pipeline 设想当作现状权威。

本目录是引擎模块（`src/engine/`）的设计文档。**目的不是描述代码现状**——代码本身才是现状的真实来源——而是承载代码无法表达的内容：意图、权衡、设计边界、跨模块的因果链。
