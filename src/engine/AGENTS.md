...
引擎设计意图通过 `src/engine/document/` 和 `docs/decisions/` 保留。工作时读取相关文档；新设计内容写入项目级 ADR，历史叙述文档只作为历史快照读取。

历史叙述文档包括 `src/engine/document/架构设计说明概要.md`、`src/engine/document/hook与触发层设计讨论结论.md`、`src/engine/document/通信协议表.md`、`src/engine/document/WorldAreaSystem.md`。
