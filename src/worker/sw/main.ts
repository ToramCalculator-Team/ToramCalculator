...
import { preCacheAll, clearOldCaches, getCacheStatus, cacheOrNetwork } from './core/cache';
import { checkVersionChange, getVersionStatus, updateStoredVersion } from './core/version';
import { isCoreResource, isAssetResource, isPageResource } from './core/fetch';
import { startPeriodicCheck, stopPeriodicCheck, getCheckStatus } from './core/periodic';
...
(async (worker: ServiceWorkerGlobalScope) => {
  ...
  function notifyAll(type: string, data: any) {
    (self as any).clients.matchAll().then((cs: readonly Client[]) => {
      cs.forEach((c: any) => c.postMessage({ type, data }));
    });
  }
