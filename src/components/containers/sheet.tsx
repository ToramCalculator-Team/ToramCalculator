...
export function SheetSurface(props: { children: JSX.Element }) {
	return <div class="Children flex flex-col gap-3">{props.children}</div>;
}
			...
			<Show when={props.state}>
				...
				>
					<div class="SheetBg bg-area-color flex h-full w-full flex-col justify-center landscape:flex-row">
						<button
							type="button"
							class={`SheetCloseBtn block flex-1 cursor-pointer backdrop-blur-xl`}
							onClick={handleClose}
						></button>
						<Motion.div
							animate={{
						...
						>
							<SheetSurface>{props.children}</SheetSurface>
						</Motion.div>
						<button
							type="button"
							class={`SheetCloseBtn hidden flex-1 cursor-pointer backdrop-blur-xl`}
							onClick={handleClose}
						></button>
					</div>
				</Motion.div>
