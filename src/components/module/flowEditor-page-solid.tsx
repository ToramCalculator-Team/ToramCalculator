...
import { SequentialWorkflowDesignerController } from "./flowEditor/SequentialWorkflowDesignerController";
    ...
    return (
      <div class="RootEditor">
        ...
        <p>
          <label>速度(毫秒)</label>
          <Input
            onInput={(e) => setProperty("speed", parseInt(e.target.value))}
  ...
  function moveViewportToFirstStepClicked() {
    ...
    if (fistStep) {
      ...
      if (controller().isReady()) {
        controller().moveViewportToStep(selectedStepId()!);
      }
    }
  }

  async function appendStepClicked() {
    const newStep = StateMachineSteps.createTaskStep("初始名称", "switch", {
      var: "测试变量",
      val: 0,
    });

    const newDefinition = _.cloneDeep(definition().value);
    ...
    <Presence exitBeforeEnter>
      <Show when={true}>
        <Motion.div
          ...
          id="FlowEditor"
        >
          <div class="basis-2/3 self-stretch">
          ...
          <div class="basis-1/3">
            ...
            <div class="w-full">
              <textarea value={definitionJson} readOnly={true} cols={30} rows={15} />
            </div>
          </div>
        </Motion.div>
      </Show>
