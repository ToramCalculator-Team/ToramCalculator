...
export const WorkflowCanvas: Component<WorkflowCanvasProps> = (props) => {
	...
	const [lastCanvasDragPosition, setLastCanvasDragPosition] = createSignal<{
		x: number;
		y: number;
	} | null>(null);

	// 获取嵌套的根节点
	const getNestedRootNodes = (
		nodes: NodeType[],
		connectors: ConnectorType[],
	): NodeWithChildren[] => {
		const addChildNodes = (parent: NodeWithChildren) => {
			// Get all outgoing connectors for the parent node.
			const outgoingConnectors = connectors.filter(
				(connector) => connector.from === parent.node.id,
			);
		...
		const rootNodes: NodeWithChildren[] = nodes
			.filter(
				(node) => !connectors.find((connector) => connector.to === node.id),
			)
	...
	const nestedRootNodes = createMemo(() =>
		getNestedRootNodes(props.nodes, props.connectors),
	);
	...
	const fit = () => {
		...
		const rootNodesContainerOffsetHeight =
			rootNodesContainerRef?.offsetHeight || 0;
		const rootNodesContainerOffsetWidth =
			rootNodesContainerRef?.offsetWidth || 0;

		setTranslateY(
			canvasWrapperOffsetHeight / 2 - rootNodesContainerOffsetHeight / 2,
		);
		setTranslateX(
			canvasWrapperOffsetWidth / 2 - rootNodesContainerOffsetWidth / 2,
		);
		setScale(1);
	};
			...
			<div class="absolute inset-0 bg-area-color">
				<svg class="absolute inset-0 w-full h-full">
					...
					<rect
						...
						width="100%"
						height="100%"
						fill="url(#canvas-pattern-background)"
					/>
				</svg>
				<div
					class="absolute"
