...
import { ToastContainer, Card } from "./components";
 ...
 */
export const BtEditor: Component = () => {
  // ==================== 状态管理 ====================
  // 布局 ID：用于标识当前加载的示例
  ...
  // Agent 类定义代码
  const [agent, setAgent] = createSignal<string>("class Agent {}");
  ...
  /**
   * 组件挂载时检查 URL 参数
   * 如果 URL 中包含 example 参数，自动加载对应的示例
   */
  onMount(() => {
    ...
    if (exampleParamValue) {
      ...
      if (example) {
        // 延迟执行，确保组件完全初始化
        setTimeout(() => {
          // 使用 MDSL 插入接口加载 example
          handleMDSLInsert(example.definition, example.board);
          setLayoutId(example.name);
        }, 10);
      }
    }
  });
  ...
  return (
      <div id="BtEditor" class="BtEditor flex w-full h-full overflow-hidden">
      <div
        class={`flex flex-col w-full sm:w-1/3 xl:w-1/4 ${
          isSidebarReadOnly() ? "opacity-60 pointer-events-none" : ""
        }`}
      >
          <DefinitionTab
      ...
      <div class="flex-1 w-full sm:w-2/3 xl:w-3/4">
        <MainPanel
          layoutId={layoutId()}
