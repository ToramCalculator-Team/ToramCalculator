...
import {
	...
	selectMemberByIdWithRelations,
} from "@db/generated/repositories/member";
import type {
	 SimulatorWithRelations,
} from "@db/generated/repositories/simulator";
import { createSignal } from "solid-js";
import { createActor, waitFor } from "xstate";
import type { ComputedSkillInfo, FrameSnapshot } from "../core/GameEngine";
import { type EngineCommand, GameEngineSM } from "../core/GameEngineSM";
...
export class Controller {
	...
	private setupDataSync() {
		...
		realtimeSimulatorPool.on(
			"engine_state_machine",
			(data: { workerId: string; event: any }) => {
	...
	private async initializeEngine(simulatorData: SimulatorWithRelations) {
		...
		console.log(
			"✅ 引擎初始化完成，当前状态:",
			this.engineActor.getSnapshot().value,
		);
	}
	 ...
	 * @param data 主控目标变化数据
	 */
	private handlePrimaryTargetChanged(data: {
		memberId: string | null;
		oldMemberId: string | null;
		timestamp: number;
	}) {
		console.log(
			`🎯 Controller: 主控目标变化 ${data.oldMemberId} -> ${data.memberId}`,
		);
	...
	private async refreshSelectedMember() {
		...
		try {
			...
			if (member.player?.characters?.[0]?.skills) {
				const skills: ComputedSkillInfo[] =
					member.player.characters[0].skills.map((skill) => ({
						id: skill.id,
