/**
 * ReactiveSystem专用AST编译器 - 精确属性路径解析
 * 
 * 专门处理ReactiveSystem中的属性表达式，所有表达式都以自身为目标
...
export class ReactiveSystemASTCompiler {
   ...
   * 收集需要替换的节点信息
   */
  private collectReplacements(ast: Program, expression: string): Array<{
    start: number;
    end: number;
    originalText: string;
    replacement: string;
    ...
    this.walkAST(ast, (node: Node) => {
      ...
      if (node.type === 'Identifier' && !this.isInMemberExpression(node, ast)) {
        ...
        if (this.knownAttributes.has(attrName) && 
  ...
  private isProtectedIdentifier(name: string): boolean {
    const protectedNames = [
      ...
      'floor', 'ceil', 'round', 'abs', 'min', 'max', 'pow',
      'true', 'false', 'null', 'undefined',
      'console', 'window', 'document', 'eval', 'Function'
    ];
   ...
   * 应用替换，从后往前避免位置偏移
   */
  private applyReplacements(expression: string, replacements: Array<{
    start: number;
    end: number;
    replacement: string;
  }>): string {
