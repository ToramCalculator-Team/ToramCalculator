...
import skillExecutionTemplate from "./skillExecutionTemplate.json";
import { magicCannonSkillEffect } from "../../types/Player/testSkills";
...
function createSkillExecutionTree(
...
): Tree<MemberBehaviorTreeRuntime<MemberStateContext>, MemberStateContext> {
  const { skillEffect, owner } = input;

  // 创建行为树上下文
  const behaviorTreeContext = new MemberBehaviorTreeRuntime(owner);
  ...
  let skillLogicTree: Tree<MemberBehaviorTreeRuntime<MemberStateContext>, MemberStateContext> | null = null;
...
export const behaviorTreeActor = fromCallback(({ input, sendBack, receive }: {
  ...
  receive((event) => {
    if (event.type === "TICK") {
      ...
      // 注意：MemberBehaviorTreeRuntime.time 是 getter，直接返回 owner.currentFrame
