...
### 2.2 Stage（阶段）：结算/计算步骤（现有 stagePool 的“stage”）
在 `runtime/Action/PipelineManager.ts` / `runtime/Action/type.ts` 中：
- `stagePool[stageName] = [inputSchema, outputSchema, impl]`
...
- 旧称 **ActionGroup（动作组）**，现统一称 **Pipeline**。
- 在 `PipelineManager` 中：`pipelineDef[pipelineName] = [stageName1, stageName2, ...]`
- 本质：**阶段的编排定义**（顺序表），不包含实现，只描述“这一组要跑哪些阶段、顺序如何”。
- 讨论“执行者产出的副作用请求”时仍称 **Intent**，避免与 Stage/Pipeline 混淆。
...
- **Stage 定义**：`src/components/features/simulator/core/Member/runtime/Action/type.ts`
  - `StagePool`：stageName -> `[inputSchema, outputSchema, impl]`
- **Pipeline 执行器**：`src/components/features/simulator/core/Member/runtime/Action/PipelineManager.ts`
  - `run(pipelineName, ctx, params?)` 执行管线
  - `compiledChains` 缓存编译后的执行链
  - `dynamicStages` 支持在静态阶段之后插入动态阶段（多用于 buff/技能特殊逻辑）
...
- `src/components/features/simulator/core/Member/Member.ts`
  ...
  - `pipelineManager`：管线执行器（Stage/Pipeline 层）
...
- `src/components/features/simulator/core/Member/runtime/Buff/BuffManager.ts`
  - buff effect 目前支持：
    ...
    - `pipeline`：通过 `PipelineManager.insertDynamicStage` 注入动态逻辑
...
- `RunPipeline`：触发某条结算管线（调用 `PipelineManager.run`）
...
- **管线内数据流**：`PipelineManager` 内的 `prevOutput/actionOutputs` 只适合“管线阶段之间传递”。
...
如果必须在代码里继续使用 `PipelineManager/stagePool` 这套命名：
