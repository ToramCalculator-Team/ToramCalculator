# 0022 - AttributeContainer 使用结构化 Modifier 来源链
...
ADR 0001 已规定所有需要 checkpoint 的持久数值统一进入 `AttributeContainer`，但 `StatusInstanceStore` 仍保留了可写的 `stacks`，形成两处都能承载层数的漂移空间。

`AttributeContainer` 当前又只按字符串 `sourceId` 保存 modifier 来源。伤害使用 `damage.hp.<areaId>`，技能消耗使用 `skill.cost.mp.<skillId>`，不同来源把因果关系编码进字符串，分析和调试需要理解各自的命名约定，还可能在伤害区域销毁后失去 `areaId` 到成员和技能的映射。
...
### B. 在 AttributeContainer 中保存真实对象引用链
...
选 C：**每个 modifier 来源由稳定 `key`、类别和从根来源到直接载体的结构化 ID 链组成；AttributeContainer 保存完整来源描述，不再把来源压缩成字符串。**
...
4. modifier 所属的 `AttributeContainer` 已表达实际目标，来源链不重复保存 `targetMemberId`。
5. 同一 `key` 再次写入时必须具有相同类别和来源链；冲突应直接报错，不允许静默合并不同来源。
6. 导出和 checkpoint 必须保存完整 `ModifierSource`，分析层不得解析 key 前缀还原因果关系。
7. 延续 ADR 0001：层数、冷却和计数器只存于 `AttributeContainer`；`StatusInstanceStore` 删除 `stacks` 和无约束 `meta`，仅保存可见状态身份、来源、标签和生命周期。

## 代价

- `AttributeContainer` 的来源条目比单个字符串占用更多内存；来源对象应按 key 复用，避免为同一来源重复创建对象。
- checkpoint 格式发生变化，旧 checkpoint 不做跨版本兼容；当前 checkpoint 只在运行会话内使用。
- 来源链要求各写入入口了解自己的直接业务来源，但不得因此把上层对象传入 AttributeContainer。

## 影响范围

- `ModifierSource`、DataStorage 导出和 `AttributeContainerCheckpoint` 使用同一结构化来源契约。
