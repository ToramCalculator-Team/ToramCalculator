...
| [0048](./0048-pglite-live-view-as-application-read-source.md) | PGlite business view 是应用正式持久数据的唯一响应式读源；repository reader 只提供 builder，应用状态不持有持久数据副本 |
...
| [0047](./0047-resource-row-owns-lifecycle-metadata.md) | 资源行直接拥有强类型生命周期时间；行为历史使用独立追加型事件存储 |
...
| [0041](./0041-character-data-uses-pglite-live-view.md) | Character 持久数据以 PGlite business view 为唯一响应式读源 | Superseded by 0048 | Character 数据 |
