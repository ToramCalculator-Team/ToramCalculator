...
2. **实时状态 SAB**：包含实体目录中的 `active`、槽位代次和稳定的 `visualProfileId`/类型引用，以及位置、朝向、生命周期、当前动画、区域形状和相机目标等动态状态。`visualProfileId` 只是资源注册表索引，不是资源定义本身。
渲染器不接收技能、攻击、受击或其他离散事件来驱动视觉状态。需要显示的持续动作必须表现为 SAB 中的当前动画状态；已经结束或未被读取到的中间状态不补播。
