...
2. **实时状态 SAB**：包含实体目录中的 `active`、槽位代次和稳定的 `visualProfileId`/类型引用，以及位置、朝向、生命周期、可重建的动画时间线、区域形状和相机目标等动态状态。动画时间线至少包含动画标识、提交时 `progress`、逻辑时间或 Tick、循环/结束状态；成员行动速度 `mspd` 是 `MemberAttrTable` 中的权威成员属性，逻辑、物理和渲染都从同一提交读取，动画只将其用于计算播放速度，不复制为第二个 `actionRate` 字段。`visualProfileId` 只是资源注册表索引，不是资源定义本身。区域状态不保存每 Tick 推导的 `position/remainingTimeMs`，而保存 `spawnTimeMs + trajectory` 轨迹描述符；移动轨迹由路径弧长与速度推导生命周期，static/attach 在轨迹内携带 `lifetimeMs`。逻辑层与渲染层共用同一 `evalTrajectory` 求值函数，在本地按当前时间计算区域位置。
...
7. 首版 `AreaStateTable` 的世界级容量为 256。区域创建超过容量时必须拒绝创建并报告诊断，不得覆盖仍然有效的区域。区域槽位保存 `spawnTimeMs + trajectory` 描述符（轨迹变体为 static / attach / segment / ray / arc / spiral），不保存每 Tick 位置或剩余时间；渲染层以同一轨迹求值函数本地计算当前帧位置。区域种类增加时继续扩展区域表的类型字段和布局版本，不复用成员表。
