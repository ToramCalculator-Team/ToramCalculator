...
- [ ] 设计草稿只保存 Player Member 的 `characterId` 以及 Mob Member 的 `mobId + mobDifficultyFlag`；进入验证时解析最新 Character/Mob 与模型资源并写入运行快照，验证或分析期间不再读取其持久数据。
- [ ] 按 ADR 0027 建立唯一场景解析边界，从 Member 引用一次产出同版本的逻辑世界输入与静态 `worldResources`；进入验证时一起冻结，3D 通过 `memberId` 将静态模型资源与引擎动态命令汇合。
- [ ] 按 ADR 0025 建立设计草稿与运行快照边界；进入验证时冻结输入，Character 持久数据变更不热更新当前运行快照。
...
先建立 Simulator 会话与成员编排边界，再按 `designing`、`simulating`、`analyzing` 三个业务阶段逐支建模。阶段由当前 Simulator 会话拥有，不是无条件存在的应用全局状态；已加载方案、运行进度、控制器关系和阶段只由显式结束或切换方案终止，Wiki 开闭和 Character 工作面切换不得改变它们。迁移时由 AUI 状态机唯一持有该会话的阶段事实，吸收并移除重复的 `businessPhaseMachine` 状态；领域会话运行时可以独立存在，但只能消费 AUI 事件或快照，不能再保存同名阶段事实。随后迁移成员选择、主控操作、进程控制和分析焦点，不复制 `worldResources` 或模拟结果。
...
- Character 修改不会改变当前 Simulator 会话的运行快照；相同快照与 seed 仍能复现相同结果。
