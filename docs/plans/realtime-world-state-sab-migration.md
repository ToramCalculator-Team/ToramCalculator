...
- **状态**: 待实施
- **日期**: 2026-08-15
- **相关决策**: ADR 0050、0052、0053；属性与历史边界依赖 ADR 0029、0044
- **目标**: 用一个版本化实时状态 SAB 替代高频 `FrameSnapshot` 和分散的渲染状态通道，让 UI 与渲染器读取同一份最新世界状态。

本计划遵守 ADR 0050、0052、0053：渲染器不接收离散视觉事件，成员动作表现由实时状态 SAB 的逻辑状态描述驱动。引擎和 UI 仍可按各自语义使用离散事件通道，但这些事件不进入渲染器。

## 范围

本计划处理实时连续世界状态及其消费者迁移：成员状态、完整成员属性、modifier 来源、区域状态、静态视觉资源注册表的接入、UI 最新视图和渲染最新视图；同时落实渲染器只由实时状态 SAB 驱动视觉表现的边界。
...
渲染层只消费两类输入：静态视觉资源注册表和实时状态 SAB。SAB 在同一提交中保存成员动作状态的状态标识、实例号和逻辑起始时间；渲染器按状态名映射到本地动画资源，并用 `logicalTimeMs - stateStartedAtLogicalTimeMs` 和本地播放策略推进当前帧，因此可以在任意稳定提交接入，不需要动作事件、缓冲或恢复回放。`mspd` 仍是通用成员属性，逻辑侧可以在触发前用它参与时长计算，但渲染器不读取它推进动画。
...
- 成员动作状态：每个逻辑提交必须写入 `stateId/stateInstance/stateStartedAtLogicalTimeMs`，不写入动画片段名、动画时长、逐帧 `progress`、`ended`、`loop` 或播放倍率。
- 属性读取：通过 `AttributeSchemaTable` 和成员局部属性数组读取 `base/act`，不为 `mspd` 保留渲染视觉专用 reader；渲染器不读取 `mspd` 推进动画。

这些数字是首版协议预算，不是允许运行时覆盖旧状态的软限制。布局不足时必须在 Worker 与 Session 协商阶段失败或重新创建新布局。

### 渲染器任意时刻接入

渲染器不参与引擎启动门控，也不区分初始化和运行中接入。静态视觉资源注册表就绪且 SAB 附件可读后，渲染器读取任意完整稳定提交，按活动槽位建立当前实体，并根据当前状态名、状态实例和逻辑起始时间重建当前表现；随后持续读取最新提交，允许跳过中间提交。

状态起始时间必须使用跨 Worker 和渲染线程可比较的逻辑时间。渲染器根据逻辑时间差和本地状态映射计算当前帧，不需要等待、缓冲、事件回放或单独的恢复协议。已经结束且未被读取到的瞬时状态不补播。
...
- [ ] 建立静态 `VisualProfileRegistry` 资源清单：模型、外观和状态到动画的映射只在渲染器本地或 Session 初始化时提供一次，不进入 SAB。
- [ ] 确认初始实体和运行中召唤物使用同一资源解析规则；预注册 `visualProfileId` 的召唤物只通过活动槽位出现，不重新请求资源清单。
- [ ] 定义成员动作状态字段：`stateId`、`stateInstance`、`stateStartedAtLogicalTimeMs`；字段必须与成员属性在同一提交中保持一致，使渲染器能够从任意稳定提交重建当前表现。
- [ ] 确认属性通过 `AttributeSchemaTable` 与成员局部属性数组读取；不建立 `mspd` 专用视觉状态接口。
- [ ] 定义渲染器任意时刻接入协议：静态资源注册表就绪后读取任意完整稳定提交，按状态描述重建当前表现并继续读取最新提交，不等待引擎就绪信号。
- [ ] 规定状态起始时间使用跨 Worker 和渲染线程可比较的逻辑时间基准；禁止使用各线程之间不可比较的本地时钟作为进度基准。
- [ ] 明确 `commitVersion`、seqlock / 原子提交规则、读失败策略和布局版本校验。
- [ ] 定义渲染器的完整提交读取接口，避免逐槽位读取导致成员、区域和动作状态来自不同提交。
...
- [ ] 在 MemberAttrTable reader 中保留布局描述符和成员局部属性数组的稳定索引契约；不要为具体属性复制专用 reader 或独立动作状态。
- [ ] 在 MemberStateTable 中实现 `stateId`、`stateInstance`、`stateStartedAtLogicalTimeMs`；不写入动画片段名、动画时长、`progress`、`ended`、`loop` 或 Babylon `speedRatio` 等渲染器本地派生值。
...
- [ ] 确保成员动作状态与成员属性在同一提交序号下可读取；渲染器按逻辑时间差和本地状态映射计算当前帧。
...
- [ ] 实现渲染器任意时刻接入逻辑：读取任意稳定 SAB 提交，按状态起始逻辑时间和本地状态映射重建当前表现；不回放已结束的历史状态。
...
- [ ] 将成员当前状态标识、实例号和逻辑起始时间写入 SAB；渲染器按同一提交的逻辑时间计算并推进当前状态对应的动画。
...
- [ ] 确认连续世界状态不再通过 `render_cmd`、`RenderSnapshot` 或其他消息对象补充；实体、区域和持续动作状态由 SAB 驱动。
- [ ] 确认 `render_cmd` 的状态同步职责和渲染器动作事件入口均已删除，不得继续承载 spawn、destroy、位姿、区域、相机或连续动作状态。
- [ ] 确认引擎与 UI 的离散事件通道不被 SAB 覆盖语义改变，且这些事件不被渲染器订阅来驱动视觉状态。
- [ ] 不建立跨线程渲染事件 envelope、全局消费确认或渲染器事件缓冲；视觉连续性由 SAB 状态描述和渲染器本地时钟负责。
...
- [ ] 删除 `render_cmd` 的状态命令生产与消费，包括 `spawn`、`destroy`、位姿校正、持续动作状态、区域同步和相机目标命令；渲染器改由静态资源注册表和实时状态 SAB 驱动。
...
- [ ] 验证渲染器在引擎启动前、运行中和重连后接入时，都能按状态起始逻辑时间和本地状态映射重建当前表现，不依赖动作事件或历史回放。
- [ ] 验证 `mspd` 属性变化不会调整已发布状态的动画播放速率，也不产生引擎逐帧动画进度消息；触发前的逻辑时长计算仍按技能规则执行。
- [ ] 验证同一提交中的状态标识、实例号和逻辑起始时间可共同重建当前表现；多个提交之间允许跳过中间状态，已结束且未被读取的瞬时状态不补播。
- [ ] 验证连续视觉表现在 60 Hz 渲染频率下保持本地连续推进，不依赖引擎逐帧消息。
