...
export const DAMAGE_RANGE_TYPE = [
	"Single", // 单体伤害,直接锁定目标ID，不依赖空间查询。
	"None", // 旧单体语义的兼容值；历史数据可能仍使用该枚举。
	"Range", // 范围伤害,伤害中心点为敌方的技能。
	"Enemy", // 周围伤害,伤害中心点为自身的技能。
	"MoveAttack", // 冲撞伤害,直线状且自身会沿攻击范围移动的技能。
	"Line", // 直线伤害,正面发出的直线状攻击。
	"Ground", // 贴地伤害,贴地移动的伤害，会受地形阻挡，表现形式通常为剑气、射刃。
	"Bullet", // 子弹伤害,向目标射出球状投射物之技能，路径中与着弹点处可以分别造成伤害。
	"GroundFixed", // 地面伤害,放置于地面上的固定伤害领域，表现形式为地刺。
	"Meteor", // 陨石伤害,自空中垂直落下的技能，表现形式为陨石。
	"Explosion", // 爆炸伤害,先在场上产生如光束落下般的区域，随后爆炸并消失的技能。
	"Attraction", // 吸引伤害,在场上产生会持续造成伤害并把玩家吸引至中心区域的领域，表现形式为漩涡或龙卷风。
] as const;
