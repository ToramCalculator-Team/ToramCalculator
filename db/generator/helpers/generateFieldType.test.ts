...
test("it respects overrides when generating field types", () => {
	...
	expect(sourceTypes.map((source) => generateFieldType(source, config))).toEqual(Object.values(overrides));
});
...
test("it supports JSON type in SQLite", () => {
	...
	expect(generateFieldType("Json", { ...config, jsonTypeOverride: "custom" })).toEqual("custom");
});
