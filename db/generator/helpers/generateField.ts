...
export const generateField = (args: GenerateFieldArgs) => {
	...
	if (generated) {
		if (isId && config.readOnlyIds) {
			fieldType = ts.factory.createTypeReferenceNode(ts.factory.createIdentifier("GeneratedAlways"), [fieldType]);
		} else {
			fieldType = ts.factory.createTypeReferenceNode(ts.factory.createIdentifier("Generated"), [fieldType]);
		}
	}
	...
	const propertySignature = ts.factory.createPropertySignature(undefined, nameIdentifier, undefined, fieldType);
