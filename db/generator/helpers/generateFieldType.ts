...
export const generateFieldTypeInner = (type: string, config: Config, typeOverride: string | null) => {
...
export const generateFieldType = (type: string, config: Config, typeOverride?: string | null) => {
	const fieldType = generateFieldTypeInner(type, config, typeOverride || null);
	if (!fieldType) throw new Error(`Unsupported type ${type} for database ${config.databaseProvider}`);
	return fieldType;
};
